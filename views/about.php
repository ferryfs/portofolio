<section id="about" class="pt-8 pb-12 md:pt-16 md:pb-20 max-w-7xl mx-auto px-6">
    ...
    <?php
    // 🔥 FUNGSI CEK GAMBAR (VERSI SOPAN & AKURAT)
    // Menggunakan __DIR__ agar PHP tidak bingung cari folder di Windows/XAMPP
    
    function getSafeImg($db_filename) {
        // 1. Cek apakah nama file kosong?
        if (empty($db_filename)) {
            return 'assets/img/default.jpg';
        }

        // 2. Cek fisik file menggunakan Absolute Path (C:\xampp\htdocs\...)
        // Kita naik satu folder (..) karena file ini ada di folder /views/
        $physical_path = __DIR__ . '/../assets/img/' . $db_filename;

        if (file_exists($physical_path)) {
            // Kalau ada, kembalikan path Relatif buat HTML
            return 'assets/img/' . $db_filename;
        } else {
            // Kalau gak ada, balikin default
            return 'assets/img/default.jpg';
        }
    }
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:h-[600px]">
        ...
        <div class="flex flex-col gap-4 h-full" data-aos="fade-right">
            ...
            <div class="flex-grow basis-0 w-full cursor-zoom-in relative group overflow-hidden rounded-[2rem] border-4 border-white bg-gray-200 shadow-sm" onclick="openImageModal('<?php echo $img1; ?>')">
                <img src="<?php echo $img1; ?>" class="w-full h-full object-cover group-hover:scale-105 transition duration-700" loading="lazy">
            </div>
            
            <div class="h-40 md:h-48 shrink-0 grid grid-cols-2 gap-4">
                <div class="h-full w-full cursor-zoom-in rounded-2xl overflow-hidden shadow-sm bg-gray-200" onclick="openImageModal('<?php echo $img2; ?>')">
                    <img src="<?php echo $img2; ?>" class="w-full h-full object-cover hover:scale-105 transition duration-500" loading="lazy">
                </div>
                <div class="h-full w-full cursor-zoom-in rounded-2xl overflow-hidden shadow-sm bg-gray-200" onclick="openImageModal('<?php echo $img3; ?>')">

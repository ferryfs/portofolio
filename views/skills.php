<section id="skills" class="pt-0 pb-8 md:pt-10 md:pb-16 max-w-7xl mx-auto px-4" data-aos="fade-up">
    <div class="island-box">
        ...
        <?php
        // LOGIC GROUPING SKILL (PHP MURNI, GAK ADA SQL)
        $skill_groups = ['Analysis' => [], 'Enterprise' => [], 'Development' => []];
        foreach($skillsData as $s) {
            $cat_db = trim($s['category']); 
            if(in_array($cat_db, ['Analysis', 'Jira', 'Design'])) $skill_groups['Analysis'][] = $s; 
            else if(in_array($cat_db, ['Enterprise', 'System', 'SAP'])) $skill_groups['Enterprise'][] = $s; 
            else $skill_groups['Development'][] = $s;
        }
        ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <?php foreach(['Analysis', 'Enterprise', 'Development'] as $cat): 
                ...
                $title_val = !empty($p['bento_title_'.$num]) ? $p['bento_title_'.$num] : $cat;

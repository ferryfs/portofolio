<?php
// apps/ess-mobile/index.php - PDO VERSION
...
// 2. QUERY ABSEN HARI INI
$today = date('Y-m-d');
$absen = safeGetOne($pdo, "SELECT * FROM ess_attendance WHERE employee_id = ? AND date_log = ?", [$nik_user, $today]);

$status_absen = 'BELUM';
if ($absen) {
    $status_absen = ($absen['check_out_time'] == NULL) ? 'SUDAH_MASUK' : 'SELESAI';
}

// 3. CEK LIBUR
$is_holiday = (date('N') >= 6); // Sabtu/Minggu
...
if ($libur) {
    $is_holiday = true;
    $holiday_reason = $libur['description'];
}
?>
...
<body>
    ...
    <div class="mobile-frame">
        <div class="app-header">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <span class="badge-role"><i class="fa fa-id-badge me-1"></i> <?php echo $role_user; ?></span>
                </div>
                <div class="bg-white text-primary rounded-circle p-2 shadow-sm"><i class="fa fa-bell"></i></div>
            </div>
        </div>

        <div class="main-card">
            <div class="row text-center">
                <div class="col-4 border-end"><h5 class="fw-bold text-success mb-0">20</h5><small class="text-muted">Hadir</small></div>
                <div class="col-4 border-end"><h5 class="fw-bold text-warning mb-0">1</h5><small class="text-muted">Telat</small></div>
                <div class="col-4"><h5 class="fw-bold text-danger mb-0">0</h5><small class="text-muted">Alpha</small></div>
            </div>
        </div>
        ...
        <div class="text-center py-2">
            ...
            <?php if ($is_holiday): ?>
                <div class="btn-checkin btn-done" onclick="alert('GAGAL: <?php echo $holiday_reason; ?>')">
            ...
            <?php elseif ($status_absen == 'SUDAH_MASUK'): ?>
                ...
                <small class="text-success fw-bold d-block mt-3">Masuk: <?php echo substr($absen['check_in_time'], 11, 5); ?></small>
            <?php else: ?>
                <div class="btn-checkin btn-done">
                    <i class="fa fa-check-circle fa-2x mb-2"></i><span>SUDAH<br>PULANG</span>
                </div>
                <small class="text-success fw-bold d-block mt-3">Pulang: <?php echo substr($absen['check_out_time'], 11, 5); ?></small>
            <?php endif; ?>
        </div>
        ...
        <div class="menu-grid">
            <a href="menu_cuti.php" class="menu-item">
                <div class="menu-icon text-warning">
                    ...
                    <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style="left: 85%!important; font-size: 0.6rem;">
                        <?php echo $sisa_cuti; ?>
                    </span>
                </div>
                <span class="text-dark">Cuti/Izin</span>
            </a>
            <a href="menu_history.php" class="menu-item"><div class="menu-icon text-secondary"><i class="fa fa-history"></i></div><span class="text-dark">Riwayat</span></a>
            <div class="menu-item" onclick="alert('Coming Soon')"><div class="menu-icon text-info"><i class="fa fa-file-medical"></i></div><span class="text-dark">Sakit</span></div>
            <div class="menu-item" onclick="alert('Coming Soon')"><div class="menu-icon text-danger"><i class="fa fa-clock"></i></div><span class="text-dark">Lembur</span></div>
            <div class="menu-item" onclick="alert('Coming Soon')"><div class="menu-icon text-success"><i class="fa fa-file-invoice-dollar"></i></div><span class="text-dark">Slip Gaji</span></div>
            <div class="menu-item"><div class="menu-icon text-primary"><i class="fa fa-tasks"></i></div><span class="text-dark">Tugas</span></div>
            <a href="menu_team.php" class="menu-item"><div class="menu-icon text-purple" style="color: purple;"><i class="fa fa-users"></i></div><span class="text-dark">Tim Saya</span></a>
            <a href="menu_setting.php" class="menu-item"><div class="menu-icon text-dark"><i class="fa fa-cog"></i></div><span class="text-dark">Setting</span></a>
        </div>
        ...
        <div class="mt-4 px-3">
            ...
            <a href="menu_approval.php" class="text-decoration-none text-dark">
                <div class="bg-white p-3 rounded-3 shadow-sm mb-2 d-flex align-items-center">
                    <div class="bg-warning bg-opacity-10 text-warning rounded-circle me-3 d-flex align-items-center justify-content-center" style="width: 40px; height: 40px;"><i class="fa fa-check-double"></i></div>
                    <div class="flex-grow-1"><h6 class="mb-0 fw-bold">Approval Cuti</h6><small class="text-muted">Kelola pengajuan tim</small></div>
        ...
        <div class="bottom-nav">
            <div class="nav-item-mobile text-primary"><i class="fa fa-home"></i><span class="nav-text">Home</span></div>
            <div class="nav-item-mobile"><i class="fa fa-calendar-alt"></i><span class="nav-text">Jadwal</span></div>
            <a href="auth.php?logout=true" class="nav-item-mobile text-decoration-none text-danger">
                <i class="fa fa-sign-out-alt"></i><span class="nav-text" style="display:block; font-size:0.7rem;">Keluar</span>
            </a>
        </div>
    </div>
    ...
    <div class="modal fade" id="modalCheckIn" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                <div class="modal-header"><h5 class="modal-title">Pilih Tipe Absensi</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
    ...
    <div class="modal fade" id="modalCheckOut" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header bg-danger text-white"><h5 class="modal-title">Laporan Kerja</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <form action="attendance.php" method="POST">
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label fw-bold">Apa yang dikerjakan hari ini?</label>
                            <textarea name="tasks" class="form-control" rows="4" placeholder="Contoh: Meeting..." required></textarea>
                        </div>
                    </div>
                    <div class="modal-footer"><button type="submit" name="checkout" class="btn btn-danger w-100 fw-bold">KIRIM & PULANG</button></div>
                </form>
            </div>
        </div>
    </div>

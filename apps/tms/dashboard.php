<?php
// apps/tms/dashboard.php (PDO FULL)
...
// FUNGSI SAFETY CHECK (PDO Version)
function safeCountTable($pdo, $table, $where = "") {
    try {
        // Cek Tabel Ada
...
// 1. DATA STATISTIK (PDO)
$total_orders = safeCountTable($pdo, "tms_orders");
$total_fleet  = safeCountTable($pdo, "tms_vehicles", "status='available'");
$total_driver = safeCountTable($pdo, "tms_drivers");
$active_ship  = safeCountTable($pdo, "tms_shipments", "status='in_transit'");

// 2. DATA LOKASI MAP (PDO)
$locations = [];
try {
    $stmt = $pdo->query("SELECT * FROM tms_locations");
    $locations = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    // Silent error kalau tabel belum ada
}
?>
...
<body>
    ...
    <div class="main-content">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="text-muted small mb-0">Welcome back, <?php echo htmlspecialchars($_SESSION['tms_fullname']); ?>!</p>
            </div>
    ...
    <div class="modal fade modal-guide" id="modalGuide" data-bs-backdrop="static" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-dark text-white border-bottom-warning">
                    <h5 class="modal-title fw-bold"><i class="fa fa-rocket me-2"></i> Selamat Datang di LogiTrack TMS</h5>
                </div>
                <div class="modal-body p-4">
                    <p class="text-muted mb-4">Halo <b><?php echo htmlspecialchars($_SESSION['tms_fullname']); ?></b>, ini adalah alur kerja sistem:</p>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="guide-step-box active">
                                <h6 class="fw-bold text-dark">1. Order Management</h6>
                                <p class="small text-muted mb-0">Terima SO dan konversi jadi Delivery Order.</p>
                            </div>
                            <div class="guide-step-box">
                                <h6 class="fw-bold text-dark">2. Fleet Allocation</h6>
                                <p class="small text-muted mb-0">Assign Driver & Kendaraan.</p>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="guide-step-box">
                                <h6 class="fw-bold text-dark">3. Live Tracking</h6>
                                <p class="small text-muted mb-0">Monitor pergerakan armada real-time.</p>
                            </div>
                            <div class="guide-step-box">
                                <h6 class="fw-bold text-dark">4. POD & Billing</h6>
                                <p class="small text-muted mb-0">Upload bukti pengiriman & invoice.</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer bg-light">
                    <button type="button" class="btn btn-primary fw-bold px-4" onclick="finishGuide()"><i class="fa fa-check me-2"></i> Mengerti</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        if (!sessionStorage.getItem('tms_guide_seen')) { modalGuide.show(); }
        function finishGuide() { sessionStorage.setItem('tms_guide_seen', 'true'); modalGuide.hide(); }
        function showGuide(force) { if(force) modalGuide.show(); }
    </script>
</body>
</html>
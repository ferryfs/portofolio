...
// V19.18: DIRECTED PUTAWAY ENGINE (SMART BIN RECOMMENDATION)
// Features: Automatic Bin Suggestion, Consolidation Logic, Overflow Management, Spaced Action Buttons
...
class InventoryModel {
    ...
    // 🔥 NEW ENGINE: findBestBin (System-Directed Logic)
    private function findBestBin($product_uuid, $batch) {
        // Priority 1: Consolidation (Same Product & Same Batch in High-Rack)
        $sql1 = "SELECT q.lgpla FROM wms_quants q 
                 JOIN wms_storage_bins b ON q.lgpla = b.lgpla 
                 WHERE q.product_uuid = ? AND q.batch = ? AND b.lgtyp = '0010' 
                 LIMIT 1";
        $res1 = safeGetOne($this->pdo, $sql1, [$product_uuid, $batch]);
        if($res1) return $res1['lgpla'];

        // Priority 2: Clean Start (Empty Bins in High-Rack)
        $sql2 = "SELECT lgpla FROM wms_storage_bins WHERE lgtyp = '0010' AND status_bin = 'EMPTY' ORDER BY lgpla ASC LIMIT 1";
        $res2 = safeGetOne($this->pdo, $sql2);
        if($res2) return $res2['lgpla'];

        // Priority 3: Fallback (Staging or Stagnant)
        return 'OVERFLOW-ZONE';
    }
    ...
    public function postGR($data) {
        try {
            ...
            if($qty_good < 0 || $qty_bad < 0) throw new Exception("CRITICAL ERROR: Negative quantities are strictly prohibited.");
            if(($qty_good + $qty_bad) <= 0) throw new Exception("Please input a valid quantity greater than zero.");

            if(!empty($data['expiry'])) {
                $today = strtotime(date('Y-m-d'));
                $input_expiry = strtotime($data['expiry']);
                if($input_expiry < $today) throw new Exception("CRITICAL ERROR: Expiry date cannot be in the past.");
            }

            $poHeader = safeGetOne($this->pdo, "SELECT status FROM wms_po_header WHERE po_number = ? FOR UPDATE", [$data['po_number']]);
            if(!$poHeader || $poHeader['status'] == 'CLOSED') throw new Exception("TRANSACTION DENIED: PO CLOSED or does not exist.");

            $doCheck = safeGetOne($this->pdo, "SELECT 1 FROM wms_gr_header WHERE po_number = ? AND vendor_do = ?", [$data['po_number'], $data['vendor_do']]);
            if($doCheck) throw new Exception("DUPLICATE ERROR: SJ Number '{$data['vendor_do']}' already processed.");
            ...
            if (($received_qty_db + $total_recv) > $qty_ordered_db) {
                ...
                throw new Exception("OVER RECEIVING DETECTED: Only $allowed remaining.");
            }
            ...
            $this->getOrCreateBin('OVERFLOW-ZONE', '9010');
            ...
            while($left > 0.0001) {
                ...
                // 🔥 FIND THE BEST BIN FOR OPERATOR
                $recommended = $this->findBestBin($item['product_uuid'], $batch_id);
                ...
                // 🔥 INSERT WITH recommended_bin
                safeQuery($this->pdo, "INSERT INTO wms_warehouse_tasks (process_type, product_uuid, batch, hu_id, source_bin, dest_bin, recommended_bin, qty, status, created_at) VALUES ('PUTAWAY', ?, ?, ?, 'GR-ZONE', 'SYSTEM', ?, ?, 'OPEN', NOW())", [$item['product_uuid'], $batch_id, $hu_id, $recommended, $take]);
            ...
            $msg = "Posted GR $gr_num. Recommended: Bin Consolidation Applied.";
...
if(isset($_POST['action'])) {
    ...
    try {
        ...
        if($_POST['action'] == 'get_gr_hus') {
            ...
            $sql = "SELECT q.hu_id, p.product_code, q.qty, p.base_uom, q.stock_type FROM wms_quants q JOIN wms_products p ON q.product_uuid = p.product_uuid WHERE q.gr_ref = ?";
...
<head>
    <meta charset="UTF-8">
    <title>Inbound Control | V19.18</title>
    ...
    <style>
        ...
        /* 🔥 FIX: Spaced Action Buttons */
        .gr-action-btn { margin-left: 0.5rem; }
    </style>
</head>
<body>
    ...
    <div class="app-sidebar">
        <div class="sidebar-brand">
            <div class="d-flex align-items-center gap-3">
                ...
                <div><h5 class="fw-bold m-0">Enterprise Inbound</h5><span class="text-muted small">WMS System V19.18</span></div>
            </div>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-end">
                                                <div class="d-flex justify-content-end gap-2">
                                                    <button class="btn btn-sm btn-info text-white rounded-pill px-3 shadow-sm" onclick="openLabelSelectModal('<?= $gr['gr_number'] ?>')"><i class="bi bi-upc-scan me-1"></i> Labels</button>
                                                    ...
                                                    <button class="btn btn-sm btn-outline-danger px-3 rounded-pill" onclick="reverseGR('<?= $gr['gr_number'] ?>')"><i class="bi bi-arrow-counterclockwise"></i> Void</button>
                                                </div>
                                            </td>
                                        </tr>
                                        <?php endforeach; ?>
                ...
                <form id="grForm">
                    ...
                    <div class="row g-4 mb-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="remarks" class="form-control form-control-lg" placeholder="Short note...">
                        </div>
                    </div>
                    <div class="p-4 rounded-4 bg-light border border-2 border-dashed">
                        ...
                        <div id="preview_content" class="pallet-grid text-center text-muted py-3">Awaiting quantity metrics...</div>
                    </div>
                </form>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 20px;">
            ...
            <div class="modal-body p-4">
                ...
                <div id="labelListContainer" style="max-height: 300px; overflow-y: auto;"></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function submitForm() {
        ...
        if(!fd.get('vendor_do')) return Swal.fire('Mandatory Field', 'Vendor SJ is required.', 'error');
        if(!fd.get('qty_good') && !fd.get('qty_bad')) return Swal.fire('Input Required', 'Enter at least one quantity.', 'warning');

        Swal.fire({ title: 'Securing Transaction...', didOpen: () => Swal.showLoading() });
        fetch('receiving.php', { method: 'POST', body: fd })
        .then(async r => { const text = await r.text(); try { return JSON.parse(text); } catch(e) { throw new Error("Invalid Server Response."); } })
        .then(d => {
            if (d.status === 'success') { Swal.fire({icon: 'success', title: 'Transaction Secured', text: `GR ${d.gr} Processed.`}).then(() => location.reload()); }
            else { Swal.fire('Validation Error', d.message, 'error'); }
    ...
    function reverseGR(grNum) {
        Swal.fire({
            ...
            text: "This will WIPE ALL staging stock and open tasks. Irreversible!",
            ...
            if (result.isConfirmed) {
                ...
                let fd = new FormData(); fd.append('gr_number', grNum);
    ...
    function openLabelSelectModal(grNum) {
        ...
        document.getElementById('btnPrintAll').onclick = function() { window.open('print_label.php?gr=' + grNum, '_blank'); };
        let fd = new FormData(); fd.append('action', 'get_gr_hus'); fd.append('gr_number', grNum);
        fetch('receiving.php', { method: 'POST', body: fd })
        .then(r => r.json()).then(res => {
            if(res.status === 'success') {
                let html = '';
                if(res.data.length === 0) { html = '<div class="alert alert-light text-center">No Handling Units found.</div>'; }
                else {
                    res.data.forEach(item => {
                        let isBad = item.stock_type === 'B6';
                        html += `<div class="d-flex justify-content-between align-items-center p-3 border rounded mb-2 ${isBad ? 'bg-danger bg-opacity-10' : 'bg-light'}">
                                    <div><div class="fw-bold font-monospace">${item.hu_id} ${isBad?'<span class="badge bg-danger ms-2">DMG</span>':''}</div>
                                    <div class="small text-muted">${item.product_code} &bull; Qty: ${parseFloat(item.qty)} ${item.base_uom}</div></div>
            ...
            } else { document.getElementById('labelListContainer').innerHTML = '<div class="text-danger">Failed to load data.</div>'; }
        });
        labelModal.show();
    }
...
// apps/wms/receiving.php
// V19.18: DIRECTED PUTAWAY ENGINE (UNCUT VERSION)
// Features: Print HU Labels Modal, Partial Receiving, Auto-Close, Native DB Remarks, Full System Audit, Precise Discrepancy Tracking, Smart Bin Recommendation
...
class InventoryModel {
    ...
    // 🔥 NEW ENGINE: Otak Pintar buat nyari Rak
    private function findBestBin($product_uuid, $batch) {
        // Priority 1: Consolidation (SKU sama & Batch sama di Rak Utama)
        $sql1 = "SELECT q.lgpla FROM wms_quants q 
                 JOIN wms_storage_bins b ON q.lgpla = b.lgpla 
                 WHERE q.product_uuid = ? AND q.batch = ? AND b.lgtyp = '0010' 
                 LIMIT 1";
        ...
        // Priority 2: Cari Rak Kosong di Rak Utama
        $sql2 = "SELECT lgpla FROM wms_storage_bins WHERE lgtyp = '0010' AND status_bin = 'EMPTY' ORDER BY lgpla ASC LIMIT 1";
        $res2 = safeGetOne($this->pdo, $sql2);
        if($res2) return $res2['lgpla'];

        // Priority 3: Kalau Penuh, Lempar ke Area Transit/Staging
        return 'OVERFLOW-ZONE';
    }
...
<head>
    <meta charset="UTF-8">
    <title>Inbound Control | V19.18 Print Label</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;700&family=JetBrains+Mono&display=swap" rel="stylesheet">
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
